  ...
  guardarRecurrente() {
    ...
    if(this.p_tdi_numero.length < 8){
      ...
      if (this.imagenrecort) {
        ...
        var p_rec_apepat = this.p_rec_apepat.toUpperCase();
        var p_rec_apemat = this.p_rec_apemat.toUpperCase();
        var p_rec_nombre = this.p_rec_nombre.toUpperCase();
        var p_tge_id = this.p_tge_id;
        var p_rec_correo = this.p_rec_correo !== null ? this.p_rec_correo : '';
        var p_rec_telfij = '';
        var p_rec_telcel = this.p_rec_telcel;
        var p_pai_id = this.p_pai_id;
        var p_udi_id = this.p_udi_id;
        var p_rec_direcc = this.p_rec_direcc.toUpperCase();
        var p_etb_id = this.p_etb_id;
        var p_etb_nombre = datoEstablecimiento.toUpperCase();
        var p_etb_direcc = this.p_etb_direcc.toUpperCase();
        var p_act_id = this.p_act_id;
        var p_act_nombre = datoRubro.toUpperCase();
        var p_ocu_id = this.p_ocu_id;
        var p_ocu_nombre = datoOcupacion.toUpperCase();
        ...
        dataPost.append('p_pai_id',p_pai_id.toString());
        dataPost.append('p_udi_id',p_udi_id.toString());
        dataPost.append('p_rec_direcc',p_rec_direcc.toUpperCase());
        dataPost.append('p_etb_id',p_etb_id);
        dataPost.append('p_etb_nombre',p_etb_nombre);
        dataPost.append('p_etb_direcc',p_etb_direcc);
        dataPost.append('p_act_id',p_act_id);
        ...
        if (result.isConfirmed) {
          this.sanidadService.guardarRecurrente(dataPost).subscribe({
            next: (data: any) => {
              let result = data[0];
              if(data.error == -3){
                Swal.fire(data.mensa, 'Vuelva a intentarlo', 'error');
